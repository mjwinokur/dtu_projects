...
#
imptypeid = []; imp_atms = []
imptypes = ['CSCCorSCCS','CCCC']
imp_E = 0.; implist = []
imp_k = [3.59,1.22]
implist.append([imptypes[0],imp_k[0],0.00000])
implist.append([imptypes[1],imp_k[1],0.00000])
...
for i in range(len(tangle1)): # Ad hoc for improper torsions
    ...
    if (('H' in temp) == False):
        ...
        if (temp2 >= 0.0 and temp2 < 1.54): 
            imp_atms.append([tangle1[i],tangle3[i],tangle2[i],tangle4[i]])
        else:
...
xwp=np.zeros(l2_num,dtype=int); ywp=np.zeros(l2_num,dtype=int); zwp=np.zeros(l2_num,dtype=int)
...
#    print i, aax[i],aay[i],aaz[i],xmin,xmax,ymin,ymax,zmin,zmax 
    for j in range(3):
        xnew[i] = aax[i]+wrapx[i]
        ynew[i] = aay[i]+wrapy[i]
        znew[i] = aaz[i]+wrapz[i]
        xmin=-Lx+(xz-xy*yz)*znew[i]+xy*ynew[i]
        xmax= Lx+(xz-xy*yz)*znew[i]+xy*ynew[i]
        ymin=-Ly+yz*znew[i]
        ymax= Ly+yz*znew[i]
        while ((aax[i]+wrapx[i]) < xmin):
            wrapx[i]=wrapx[i]+a[0]
            xwp[i] -= 1
            if ((aax[i]+wrapx[i]) >= xmin):
                break
        while ( (aax[i]+wrapx[i]) > xmax):
            wrapx[i]=wrapx[i]-a[0]
            xwp[i] += 1
            if ((aax[i]+wrapx[i]) <= xmax):
                break
        while ((aay[i]+wrapy[i])  < ymin):
            wrapy[i]=wrapy[i]+b[1]
            ywp[i] -= 1
            if ((aay[i]+wrapy[i]) >= ymin):
                break
        while ((aay[i]+wrapy[i]) > ymax):
            wrapy[i]=wrapy[i]-b[1]
            ywp[i] += 1
            if ( (aay[i]+wrapy[i]) <= ymax):
                break
        while ((aaz[i]+wrapz[i]) < zmin):
            wrapx[i]=wrapx[i]+c[0]
            wrapz[i]=wrapz[i]+c[2]
            zwp[i] -= 1
            if ((aaz[i]+wrapz[i]) >= zmin):
                break
        while ((aaz[i]+wrapz[i]) > zmax):
            wrapx[i]=wrapx[i]-c[0]
            wrapz[i]=wrapz[i]-c[2]
            zwp[i] += 1
            if ((aaz[i]+wrapz[i]) <= zmax):
                break
#
...
io_hoomd_params('hoomd_pars.dat','w',bdtypes_tot,angtypes_tot,tortypes_tot,lj_pair_1,lj_pair_2,bond1,bond2,
                uc,astring,atype,abt,header,epsilon,sigma,angk,angt0,tor1,tor2,tor3,tor4,dipole_par2,dipole_types,implist)
...
u = gsd.hoomd.open('test.gsd', mode='wb') # No wrapping for periodic boundary conditions
u.append(s)
s.particles.position = [(xnew[i]*ang2nm,ynew[i]*ang2nm,znew[i]*ang2nm) for i in range(l2_num)]
s.particles.image = [(xwp[i],ywp[i],zwp[i]) for i in range(l2_num)]
u = gsd.hoomd.open('test2.gsd', mode='wb')
u.append(s)
#for i in range(l2_num):
