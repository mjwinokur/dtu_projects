...
#
def peak_map(ii,j0,i0,alphai,tth,htth,lambdaa,SSD):
#    function [qi,qo]=PeakMapping(j,i,j0,i0,alphai,tth,htth,lambda,LSD)
# PEAKMAPPING Maps pixel values (j,i) to Q values (Qi,Qo).
# 
# INPUT:
# j,i = coordinates in pixel values (col and row)
# j0,i0 = col and row of the normal point
# alphai = incidence angle in degrees
# tth, htth = detector angles (vetical and horizontal) in degrees
# lambda = Wavelength of x-rays in Å
# LSD = Detector distance in Å
# OUTPUT:
# qi,qo = coordinates in q values (in-plane and out-of-plane)
#
# convert angles to rad
    alphai=np.deg2rad(alphai);tth=np.deg2rad(tth);htth=-np.deg2rad(htth)
# define image coordinates
#    print i
#    print j
    xp =-np.array(ii,dtype=float)
#    yp =-np.array(ii[2,:],dtype=float)
#    print xp
#    print yp
    xp +=  [j0,i0]
#    yp +=  i0
...
# wavenumber
    k = 2.0*np.pi/lambdaa
#  incoming wavevector ki in lab basis
##    kiL = k*np.array([1.0,0.0,0.0])
##    cosalphai=math.cos(alphai)
##    sinalphai=math.sin(alphai)
#    print kiL
# rotation matrix (eq.31)
##    AiT=np.array([[cosalphai,0.0,sinalphai],[0.0,1.0,0.0],[-sinalphai,0.0,cosalphai]])
# incoming wavevector ki in sample basis
#    kiS = np.linalg.solve(kiL,AiT) #kiS=AiT\kiL;
##    kiS = np.linalg.solve(AiT,kiL) #kiS=AiT\kiL;
#    print 'kiS:',kiS
#outgoing wavevector kf in lab basis (eq.24)
    sinhtth = math.sin(htth)
    coshtth = math.cos(htth)
    sintth = math.sin(tth)
    costth = math.cos(tth)
    qi=[]
    qo=[]
...
#        print x,y
# normalization 
        deltak = 1./np.sqrt(x*x+y*y+1.)
        kdeltak = k*deltak
#        print 'Dk:',deltak,kdeltak
#        print 'x*coshtth...',x*coshtth,-sinhtth*costth,+y*sinhtth*sintth
#        print 'sum',x*coshtth-sinhtth*costth+y*sinhtth*sintth
        Qx =   kdeltak*(x*coshtth-sinhtth*costth+y*sinhtth*sintth) 
        Qy = k*(deltak*(x*sinhtth+coshtth*costth-y*sintth*coshtth)-1.)
        Qz = kdeltak*(sintth+y*costth)
#        print 'Qs:',Qx,Qy,Qz

# Q_in_plane and Q_out_of_plane
        qi.append(np.sqrt(Qx*Qx+Qy*Qy))
#        qo.append(np.abs(Qz))
        qo.append(Qz)
    return qo,qi
#
...
# Test above
nx=1024; ny=1024
xi = np.linspace(1, 2*nx, num=nx, endpoint=True)
yi = np.linspace(1, 2*ny, num=ny, endpoint=True)
XI, YI = np.meshgrid(xi, yi)

###qxy =np.zeros([1024,1024],dtype=float)
###qz =np.zeros([1024,1024],dtype=float)
lambdaa=1.24
SSD = 3.192e9
j0 = 1000
i0 = 1020
...
for k in range(1,1024):
    ...
    for m in range(1,1024):
        m2=2*m-1
        ii.append([k2,m2])
ii = [[576,1711],[1024,2048],[2048,1024],[2048,2048]]
qo,qi = peak_map(ii,j0,i0,alphai,tth,htth,lambdaa,SSD)
for k in range(len(qo)):
    print k,ii[k],qi[k],qo[k]
i=[0]
j=[0]
qo,qi = peak_map(i,j,j0,i0,alphai,tth,htth,lambdaa,SSD)
for k in range(len(qo)):
    print k,qi[k],qo[k]
#        qxy[i,j]=q0
#        qz[i,j]=qi
...
#        print mylist
        if (mylist[0] != 'h' and mylist[0] != '+' and alen == 6):
            hkl.append([float(mylist[0]),float(mylist[1]),float(mylist[2])])
            I.append(float(mylist[4]))
            dspace.append(float(mylist[3]))
            mult.append(int(mylist[5]))
    file.close()
#    l2_num=l2_num-1
    return hkl,I,dspace,mult
#
...
#         print 'test.py -i <inputfile> -o <outputfile> -t <"special options">'
         ...
         text_ctrl = str(arg)
#   print 'Input file is "', inputfile;print 'Output file is "', outputfile; print 'Text is "', text_ctrl
...
#	print ict,mylist 
        if (mylist[0] != '#'): # skip comments
            xdata.append(float(mylist[0]))
            array=[]
            for i in range(1,len(mylist)):
                array.append(float(mylist[i]))
            ydata.append(array)
    return xdata,ydata
#
...
#	print ict,mylist 
        if (mylist[0] == '#' and new_set == 'false'): # skip comments
            new_set = 'true'
            ict = 0
            hval.append(mylist[1])
            kval.append(mylist[2])
            lval.append(mylist[3])
        elif (mylist[0] == '#' and ict > 0):
            ict=0
            xdata.append(xtemp)
            ydata.append(ytemp)
            xtemp=[]
        ...
        if (mylist[0] != '#'): 
            ict += 1
            xtemp.append(float(mylist[0]))
            ytemp.append(float(mylist[1]))
            new_set == 'false'
    xdata.append(xtemp)
    ydata.append(ytemp)
